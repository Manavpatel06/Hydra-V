...
import { expectedImprovement, predictGaussianProcess, trainGaussianProcess } from './gaussianProcess'
import { clamp, mean, stdDev } from './math'
import { compositeOutcomeScore } from './scoring'
...
const PARAMETER_LABELS: Record<keyof ProtocolParameters, string> = {
...
export const defaultProtocol = (): ProtocolParameters => ({
  lightRatio: 0.58,
  vibrationHz: 44,
  thermalGradient: 0.5,
  padDurationMin: 16,
...
const normalizeProtocol = (protocol: ProtocolParameters): number[] => [
...
const denormalizeProtocol = (features: number[]): ProtocolParameters => ({
  lightRatio: clamp(features[0], 0, 1),
  vibrationHz: Math.round(
    MIN_VIBRATION + clamp(features[1], 0, 1) * (MAX_VIBRATION - MIN_VIBRATION),
  ),
  thermalGradient: clamp(features[2], 0, 1),
  padDurationMin: Math.round(
    MIN_DURATION + clamp(features[3], 0, 1) * (MAX_DURATION - MIN_DURATION),
  ),
  resonanceBias: clamp(features[4], 0, 1),
  contralateralTargeting: features[5] >= 0.5,
})

const inferModality = (protocol: ProtocolParameters): SessionModality => {
...
const candidateSet = (latest: ProtocolParameters, size: number, seed: number): number[][] => {
...
const plateauSignal = (
  ...
  if (key === 'vibrationHz') {
    ...
    return `Vibration response is plateauing near ${Math.round(
      baseline,
    )} Hz. Try ${suggestion} Hz or add contralateral targeting next session.`
  }

  if (key === 'thermalGradient') {
    const suggestion = clamp(baseline + 0.14, 0, 1)
    return `Thermal adaptation is flattening near ${baseline.toFixed(
      2,
    )}. Increase gradient toward ${suggestion.toFixed(2)} for exploration.`
  }

  if (key === 'lightRatio') {
    const suggestion = clamp(baseline + 0.12, 0, 1)
    return `Light ratio impact is flattening near ${baseline.toFixed(
      2,
    )}. Test a shift toward ${suggestion.toFixed(2)} for improved response.`
  }
...
export const recommendProtocol = (
  sessions: SessionRecord[],
  usingWebGPU: boolean,
): ProtocolRecommendation => {
  ...
  return {
    ...
    rationale: [
      ...
      `Predicted composite gain ${bestMean.toFixed(
        2,
      )} with targeted exploration to avoid local plateaus.`,
